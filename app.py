...
st.session_state.cat_df          = cat_df_rawst.session_state.settings_df     = settings_df_raw
...
if "show_modal" not in st.session_state:
    st.session_state.show_modal = Falseif "form_id" not in st.session_state:
...
if "pin_attempts" not in st.session_state:
    st.session_state.pin_attempts = 0if "pin_error" not in st.session_state:
...
def delete_expense(idx):
    with st.spinner("Deleting..."):
        ...
        conn.update(worksheet="Expenses", data=updated)
        st.session_state.df = updateddef save_settings(new_df):
    with st.spinner("Saving..."):
...
if not st.session_state.get("auto_log_checked") and not st.session_state.settings_df.empty:
    ...
    for i, row in st.session_state.settings_df.iterrows():
        try:
            ...
            updated_sdf.at[i, "Last_Fired"] = curr_ym            fired_any = True
        except:
            pass
    if fired_any:
...
if "🏠 Home" in page:
    ...
    # ── Quarterly card ───────────────────────────────    q_months_str = ["Jan–Mar","Apr–Jun","Jul–Sep","Oct–Dec"][curr_q-1]
...
# PAGE: CATEGORIES# ═══════════════════════════════════════════════════════
    ...
    # Add new    with st.form("add_cat_form"):
...
# ═══════════════════════════════════════════════════════elif "⚠️ Review" in page:
    ...
    if active_pend.empty:
        ...
        for idx, row in active_pend.iterrows():
            ...
            # Category selector — suggestion pre-selected if available            cat_opts      = ["-- Select --"] + live_cats + ["+ New category"]
            default_idx   = 0
            ...
            final_cat  = None
            is_new_cat = False            if sel == "+ New category":
            ...
            # Action buttons in one row            b1, b2, b3, b4 = st.columns(4)
    ...
    if not apps_script_url:
        ...
        if "sync_result" in st.session_state:
            ...
            if r.get("status") == "ok":
                ...
                st.error(f"Sync failed: {r.get('message','Unknown error')}")             del st.session_state["sync_result"]
