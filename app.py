...
def save_modes(new_df):
    with st.spinner("Saving..."):
        ...
        st.session_state.modes_df = new_dfdef save_pin(new_pin: str):
...
def approve_pending_row(idx, chosen_category, create_new_cat=False):
    with st.spinner("Approving..."):
        ...
        st.session_state.df = updated_exp        updated_pend = st.session_state.pending_df.drop(idx).reset_index(drop=True)
...
for _k, _v in [("pin_unlocked",False),("pin_input",""),("pin_attempts",0),("pin_error","")]:
    if _k not in st.session_state:
        st.session_state[_k] = _vif not st.session_state.pin_unlocked:
    locked_out = st.session_state.pin_attempts >= MAX_PIN_ATTEMPTS    st.markdown("<br>", unsafe_allow_html=True)
...
if "🏠 Home" in page:
    ...
    q_pct        = min(qtr_total / q_budget * 100, 100) if q_budget > 0 else 0    q_color      = "#e53935" if q_pct > 90 else ("#f57c00" if q_pct > 70 else "#43a047")
